...
var core = {
  config: index_js.runtimeConfig,
  currentContext: index_js.getContextOwner,
  root: index_js.createRoot,
  ignore: index_js.sample,
  effect: index_js.createEffect,
  memo: (fn, equal) => {
    if (!equal) return index_js.createMemo(fn);
...
function Suspense(props) {
  let counter = 0,
      ...
      showFallback,
      transition;
  const [state, nextState] = index_js.createSignal("running", index_js.equalFn),
        store = {
    increment: () => {
      if (++counter === 1) {
        if (!store.initializing) {
          if (index_js.SuspenseContext.transition) {
            ...
            t = setTimeout(() => nextState("fallback"), index_js.SuspenseContext.transition.timeoutMs);
            nextState("suspended");
    ...
    decrement: () => {
      if (--counter === 0) {
        ...
        nextState("running");
        index_js.afterEffects(() => index_js.SuspenseContext.decrement());
      }
    },
    state,
    initializing: true
...
function For(props) {
  ...
  return index_js.suspend(index_js.createMemo(index_js.mapArray(() => props.each, props.children, fallback ? fallback : undefined)));
}
function Show(props) {
  const useFallback = ("fallback" in props),
        condition = index_js.createMemo(() => !!props.when, undefined, index_js.equalFn);
  return index_js.suspend(index_js.createMemo(() => condition() ? index_js.sample(() => props.children) : useFallback ? index_js.sample(() => props.fallback) : undefined));
}
function Switch(props) {
  ...
  return index_js.suspend(index_js.createMemo(() => {
